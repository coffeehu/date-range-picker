<template>
  <div
    class="date-range-picker czy-ui el-date-editor"
    :class="[
      'el-date-editor--' + type,
...
import { formatDate, parseDate, isDateObject, getWeekNumber } from 'element-ui/lib/utils/date-util';
...
const TYPE_VALUE_RESOLVER_MAP = {
  ...
  yearrange: {
    formatter(value, format) {
      if (Array.isArray(value) && value.length === 2) {
        const start = value[0].getFullYear();
        const end = value[1].getFullYear();
...
export default {
  ...
  methods: {
    ...
    mountPicker() {
      ...
      this.picker.$on('pick', (date = '', visible = false, isYear = false) => {
        if (isYear) {
          date[0] = new Date(date[0]+'')
          date[1] = new Date(date[1]+'')
        }
        console.log(111111111, date, visible, isYear)
        this.userInput = null;
